...
func _physics_process(_delta: float) -> void:
	...
	if closestTarget != null:
		(closestTarget.position - body.position).normalized() * statBlock.movespeed # move towards target
		...
		if body.is_in_group("Friendly") and body.position.x >= get_viewport_rect().size.x * 2/3:
